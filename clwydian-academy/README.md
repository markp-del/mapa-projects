...
- **Update the business plan**: replace `assets/docs/Charlotte-Eleanor-Group-Business-Plan.pdf` with a new file *using the same filename* — every link keeps working.
...
- **Add the company number**: once the CIC is incorporated, append it to the legal line in the footer of every page (search for `Community Interest Company —`).
